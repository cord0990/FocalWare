...
| **RF-05** | Cálculo de índice ponderado | El sistema debe calcular un índice ponderado según categoría, volumen, reportes cercanos, antigüedad y condiciones meteorológicas externas. | Algoritmos | Ninguna | Sistema |
...
| **RF-09** | Estadísticas mensuales | El sistema debe permitir al Funcionario acceder a una sección de estadísticas que muestre un resumen mensual con el total de reportes, los reportes resueltos, el tiempo promedio de resolución y la evolución mensual. | Niveles de Autorizacion | Ninguna | Funcionario |
| **RF-10** | Identificador unico de reportes | El sistema debe generar y asignar automáticamente un identificador único para cada reporte creado. | Regla de negocio | RF-01 | Sistema |
| **RF-11** | Ocultamiento automático de reportes controlados | El sistema deberá ocultar del mapa los reportes en estado "Controlado" después de un mes, manteniendo su visualización de forma exclusiva para los usuarios con rol de Funcionario. | Niveles de autorización | RF-03, RF-12 | Vecino / Funcionario |
| **RF-12** | Opciones de Funcionario en reportes | El sistema deberá permitir a los funcionarios acceder a un menú de opciones para cada reporte que incluya: aceptar, rechazar, modificar o generar un reporte de control. | Niveles de autorización | RF-01, RNF-09 | Funcionario |
| **RF-13** | Modificacion Offline | El sistema debe permitir modificar los reportes guardados en el almacenamiento local que aún no han sido enviados | Transacciones | RF-01, RF-02 | Vecino / Funcionario |
| **RF-14** | Votos a favor de reportes | El sistema debe permitir a los usuarios aumentar la relevancia de un reporte mediante un sistema de votos a favor (upvotes). | Reglas de negocio / Transacciones | RF-01, RF-03, RF-15 | Vecino / Funcionario |
| **RF-15** | Vista de Detalles | El sistema debe mostrar para cada reporte la siguiente información de ubicación, sector, ID, fotografías (hasta 5), categoría de residuo, volumen estimado, votos a favor, descripción, riesgo ponderado y condiciones meteorológicas del día de creación. | Reglas de negocio | RF-01, RF-14, RF-15 | Vecino / Funcionario |
| **RF-16** | Limitacion de reporte controlado | El sistema debe permitir a un funcionario crear un reporte controlado únicamente si cuenta con la autorización para formar parte de la cuadrilla asignada. | Niveles de autorización | RF-01, RF-12 | Funcionario |
| **RF-17** | Asignacion de reporte | El sistema debe permitir a los funcionarios asignar a un reporte aprobado una cuadrilla, una fecha de acción y una descripción opcional. | Niveles de autorización / Transacciones | RF-01, RF-12 | Funcionario |
| **RF-18** | Motivos de Rechazo | El sistema debe exigir a los funcionarios asignar a un reporte rechazado una descripción obligatoria que indique el motivo del rechazo. | Niveles de autorización / Transacciones | RF-01, RF-12 | Funcionario |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **RNF-01** | Límite de pasos de reporte | El usuario debera crear un reporte en maximo 2 pasos. | Usabilidad | RF-01 | Vecino |
...
| **RNF-08** | Validación de duplicados y orden de prioridad | El sistema deberá validar que los reportes subidos por los usuarios a la plataforma no se encuentren en existencia en la base de datos, si existe mandara un mensaje de "Ya existe un antecedente previo de este reporte" al usuario, incorporando un sistema de Upvotes para que se suban por orden de prioridad. | Integridad de los datos | RF-01, RF-02, RF-14, RNF-3 | Vecino |
